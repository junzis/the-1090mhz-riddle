...
When the Q bit is set to \texttt{0}, an alternative encoding with 100 feet increments is used. The remaining bits are encoded using the Gillham code, a reflected Gray-code variant that maps directly onto the output of legacy pressure altimeters \cite{doran2007}. This is not an indicator of high altitude; it is simply a different encoding standard. The full decoding procedure is described in Section~\ref{sec:alt_code}.
